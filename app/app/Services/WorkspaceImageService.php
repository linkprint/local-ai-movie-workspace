...
class WorkspaceImageService
{
    ...
    private const EXTENSIONS = ['gif', 'jpeg', 'jpg', 'png', 'webp'];
    ...
    public function imagesFor(WorkspaceProfile $profile, WorkspaceProject|string $scope): array
    {
        ...
        foreach ($iterator as $file) {
            ...
            if (! is_string($relative) || ! $this->isImageName($relative)) {
                continue;
            }
            $images[] = [
                'path' => $relative,
        ...
        usort($images, fn (array $left, array $right): int => $right['modified_at'] <=> $left['modified_at']);
    ...
    public function rename(
        ...
        if ($newName === ''
            ...
            || ! $this->isImageName($newName)) {
            throw ValidationException::withMessages([
                'new_name' => __('ui.image_library_errors.name_rules'),
            ]);
        }
        ...
        if (file_exists($destination) || is_link($destination)) {
            throw ValidationException::withMessages(['new_name' => __('ui.image_library_errors.name_duplicate')]);
        }
    ...
    public function trashBatch(WorkspaceProfile $profile, array $identifiers): array
    {
        ...
        foreach ($items as $item) {
            ...
            if (file_exists($destination) || is_link($destination)) {
                throw new RuntimeException('image_trash_unavailable');
            }
    ...
    private function storageRoot(WorkspaceProfile $profile): string
    {
        ...
        if (! Str::isUuid($storage)) {
            throw new RuntimeException('invalid_image_storage');
        }
    ...
    private function scopeName(WorkspaceProject|string $scope): string
    {
        ...
        if ($value !== self::LEGACY_SCOPE && ! Str::isUuid($value)) {
            throw new RuntimeException('invalid_image_scope');
        }
    ...
    private function isImageName(string $path): bool
    {
    ...
    private function urlFor(WorkspaceProject|string $scope, string $relative, bool $download = false): string
    {
        ...
        $route = $scope instanceof WorkspaceProject
            ? '/workspace/projects/'.$scope->id.'/images/'.$encoded
            : '/workspace/images/legacy/'.$encoded;
